...
DOCKER_TAG="puma-challenge-baseline-track1"
...
docker run --rm \
    ...
    alpine:latest \
    sh -c "chown -R 1000:1000 /output && chmod -R 777 /output"
...
EXPECTED_FILE_NUCLEI="/output/melanoma-3-class-nuclei-segmentation.json"
...
if docker run --rm -v puma-output:/output/ python:3.7-slim sh -c "[ -f $EXPECTED_FILE_NUCLEI ]"; then
    ...
    docker cp "$TEMP_CONTAINER_NUCLEI:$EXPECTED_FILE_NUCLEI" "$SCRIPT_DIR/output/melanoma-3-class-nuclei-segmentation.json"
    echo "Copied .json file to host: $SCRIPT_DIR/output/melanoma-3-class-nuclei-segmentation.json"
...
docker run --rm -v puma-output:/output alpine:latest sh -c "ls -R /output"
...
# import tifffile
#
#file_path = '/output/${INPUT_FILENAME}.tif'
